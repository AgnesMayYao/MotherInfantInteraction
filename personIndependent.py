...
def getOutput(sessionData):
	output = sessionData.split('(')[1][:-1]
	output = output.replace(" ",'')
	output = output.replace(".", '')
	return output


def giant_plot(time, data, label, predict,output_directory, file_for_testing, windowSize = str(1), special = ''):
	...
	prefilter.plot(time, data[:,14],'green')
...
def ml(X_train, X_test,Y_train, Y_test, num_estimators):
    ...
    clf = RandomForestClassifier(n_estimators = num_estimators, criterion = 'entropy')
...
if __name__ == '__main__':
	...
	for file_for_testing in files_for_testing:
		...
		training_data = np.array(training_data)[:, [0, 3, 4, 7, 10, 11, 12, 13, 14]]
		test_data = np.array(test_data)[:, [0, 3, 4, 7, 10, 11, 12, 13, 14]]
		for i in range(5,len(training_label[0])):

			for k in range(10, 50, 3):
				RF, prediction=ml(np.array(training_data),np.array(test_data),np.array(training_label)[:,i],np.array(test_label)[:,i], k)
			...
			#giant_plot(np.array(test_label)[:,0], np.array(test_data), np.array(test_label)[:,i], prediction,"./personIndependent_newStates_baseline_RF/", file_for_testing, special)
